...
| **Version gate** | Pin the Minecraft versions your network accepts via `conduit.toml → [versions]`. In-range clients see the normal MOTD, ping, and player count; out-of-range clients get the vanilla "incompatible version" server-list entry labelled with your supported versions, and a join attempt is refused with a message naming them. |
...
minimum            = "1.21.11"  # version name or protocol number; "" for no lower bound
